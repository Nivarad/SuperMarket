...
int main(int argc, char* argv[])
{
	...
	errno = 0;
	isCompressed = strtol(argv[2], &p, 10);
	char* fileName = argv[1];
	if (!initSuperMarket(&market, fileName, isCompressed, CUSTOMER_FILE_NAME))
	{
	...
	if (!saveSuperMarketIntoCompressedFile(&market, SUPER_COMPRESS_FILE_NAME, CUSTOMER_FILE_NAME))
