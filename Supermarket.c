...
int		initSuperMarket(SuperMarket* pMarket, const char* fileName, int compressedFlag,
...
int		addProduct(SuperMarket* pMarket)
{
	...
	if(res==1) //This barcode exist in stock
...
int		addCustomer(SuperMarket* pMarket)
{
	...
	Customer* temp = NULL;
	temp= (Customer*)realloc(pMarket->customerArr, (pMarket->customerCount + 1) * sizeof(Customer));
...
int	doShopping(SuperMarket* pMarket)
{
	...
	if (pCustomer->pCart == NULL)
	{
		...
		int res = 1;
		CHECK_RETURN_0(pCustomer->pCart, res);
		if (res==0)
			return 0;
...
void fillCart(SuperMarket* pMarket,ShoppingCart* pCart)
{
	...
	while (1)
	{
		...
		garbage=getchar(); //clean the enter
		if (op != 'y' && op != 'Y')
			break;
		int count;
...
void findCustomer(const SuperMarket * pMarket)
{
	...
	case eTime:
		...
		garbage=scanf("%d", &cust.shopTimes);
		break;

	case eSpend:
		printf("Enter spent amount\n");
		garbage=scanf("%f", &cust.totalSpend);
		break;
	}
